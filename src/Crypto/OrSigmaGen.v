...
Section DL. 
  ...
  Section Or.
    ...
    Section Defs.
      ...
      (* This file is a generalisation of OrSigma.v *)
      ...
      *)
        ...
        *)
        ...
        Proof.
          ...
          destruct (splitat m gs) as (gsl & gsrh).
          destruct (splitat m hs) as (hsl & hsrh).
          destruct (vector_inv_S gsrh) as (g & gsr & _).
          destruct (vector_inv_S hsrh) as (_ & hsr & _).
          (* left simulate commitments *)
          set (coml := zip_with (fun ui ci => (ui, ci)) usl csl).
          set (commitl := zip_with (fun '(gi, hi) '(ui, ci) => 
            gop (gi^ui) (hi^(opp ci))) (zip_with pair gsl hsl) coml).
          (* real commitment *)
          set (com := g^u). 
          (* right simulate commitment *)
          set (comr := zip_with (fun ui ci => (ui, ci)) usr csr).
          set (commitr := zip_with (fun '(gi, hi) '(ui, ci) => 
            gop (gi^ui) (hi^(opp ci))) (zip_with pair gsr hsr) comr).
        ...
        Proof.
          (* commitment *)
          set (comm := @construct_or_conversations_schnorr_commitment m n gs hs 
            uscs).
          destruct (splitat (m + (1 + n)) uscs) as (us & cs).
          (* us is the randomness for commitment and cs is the degree of freedom *)
          (* split us for simulation  *)
        ...
        Proof.
          ...
          pose proof (@construct_or_conversations_schnorr m₁ m₂ x (rew pfa in gs) 
        ...
        Proof.
          ...
          set(commitment := zip_with (fun '(gi, hi) '(ui, ci) => 
            gop (gi^ui) (hi^(opp ci))) (zip_with pair gs hs) comm).
        ...
        Proof.
          ...
          refine
            ...
            | left _ => 
                (* now check sigma *)
                or_accepting_conversations_supplement gs hs (a; cs; r)
        ...
        Definition generalised_or_schnorr_distribution  
          ...
          (rindex : Fin.t (2 + n)) (x : F) (gs hs : Vector.t G (2 + n)) (c : F) : 
          dist (@sigma_proto F G (2 + n) (1 + (2 + n)) (2 + n)) := 
          uscs <- repeat_dist_ntimes_vector 
            (uniform_with_replacement lf Hlfn) ((2 + n) + (1 + n)) ;;
          Ret (generalised_construct_or_conversations_schnorr rindex x gs hs uscs c).
        ...
        Definition generalised_or_simulator_distribution  
          ...
          usrs <- repeat_dist_ntimes_vector 
            (uniform_with_replacement lf Hlfn) ((2 + n) + (1 + n)) ;;
          Ret (generalised_construct_or_conversations_simulator gs hs usrs c).
    ...
    Section Proofs.
        ...
        *)
        Lemma or_accepting_conversations_correctness_supplement_forward : 
          ...
          or_accepting_conversations_supplement gs hs s = true ->
          (match s with 
          | (a; c; r) => 
            ∀ f : Fin.t n, 
        ...
        Lemma or_accepting_conversations_correctness_supplement_forward_reject : 
          forall {n : nat} (gs hs : Vector.t G n)
          (s :  @sigma_proto F G n n n),
          or_accepting_conversations_supplement gs hs s = false ->
          (match s with 
          | (a; c; r) => 
            ∃ f : Fin.t n, 
            @accepting_conversation F G gop gpow Gdec (nth gs f) (nth hs f) 
        ...
        Proof.
          ...
          +
            ...
            destruct (vector_inv_S r₁) as (rh₁ & rtl₁ & Hg).
            subst. cbn in ha.
            destruct (Gdec (gsh ^ rh₁) (gop ah₁ (hsh ^ ch₁))) as [hb | hb].
            ++
              destruct (ihn gstl hstl (atl₁; ctl₁; rtl₁) ha) as (f & hc).
        ...
        Lemma or_accepting_conversations_correctness_supplement_backward : 
          ...
          | (a; c; r) => 
            ...
            @accepting_conversation  F G gop gpow Gdec (nth gs f) (nth hs f) 
              ([(nth a f)]; [nth c f]; [(nth r f)]) = true
          end) -> or_accepting_conversations_supplement gs hs s = true.
        Proof.
          induction n as [|n IHn].
          +
            intros * Ha.
        ...
        Lemma or_accepting_conversations_correctness_supplement_backward_reject : 
          ...
          | (a; c; r) => 
            ...
            @accepting_conversation  F G gop gpow Gdec (nth gs f) (nth hs f) 
              ([(nth a f)]; [nth c f]; [(nth r f)]) = false
          end) -> or_accepting_conversations_supplement gs hs s = false.
        Proof.
          induction n as [|n ihn].
          +
            intros * ha.
        ...
        Lemma or_accepting_conversations_supplement_app :
          ...
          or_accepting_conversations_supplement
            (gsl ++ gsr) (hsl ++ hsr) 
            ((al ++ ar); (cl ++ cr); (rl ++ rr)) = 
          or_accepting_conversations_supplement gsl hsl (al; cl; rl)  && 
          or_accepting_conversations_supplement gsr hsr (ar; cr; rr).
        Proof.
          induction n as [|n IHn].
          +
            intros *.
        ...
        Lemma generalised_or_accepting_conversations_correctness_forward_reject : 
          forall {n : nat} (gs hs : Vector.t G n )
          (s :  @sigma_proto F G n (1 + n) n),
          generalised_or_accepting_conversations gs hs s = false -> 
          match s with 
          | (a; c; r) => 
            Vector.hd c ≠ Vector.fold_right add (Vector.tl c) zero ∨
            (∃ (f : Fin.t n),
        ...
        Theorem construct_or_conversations_schnorr_completeness_base : 
          ...
          (hsr gsr : Vector.t G n),
          gsr[@f] ^ usr[@f] = gop 
          (zip_with (λ '(gi, hi) '(ui, ci), gop (gi ^ ui) (hi ^ opp ci)) 
          (zip_with pair gsr hsr)
        ...
        Proof.
          ...
          +
            ...
            destruct (vector_inv_S gsr) as (gsrh & gsrt & hd).
            subst.
        ...
        Theorem construct_or_conversations_schnorr_completeness_generic : 
          forall (m n : nat) (f : Fin.t (m + S n))  (usl : Vector.t F m)
          (c : F) (csl : Vector.t F m) (csr : Vector.t F n) (x : F) (u : F)
          (usr : Vector.t F n) (hsl : Vector.t G m) (h : G) (hsr : Vector.t G n)
          (gsl : Vector.t G m) (g : G) (gsr : Vector.t G n), g^x = h -> 
          (gsl ++ [g] ++ gsr)[@f] ^ (usl ++ u + (c - fold_right add (csl ++ csr) zero) * x :: usr)[@f] =
          gop (zip_with (λ '(gi, hi) '(ui, ci), gop (gi ^ ui) (hi ^ opp ci)) 
          (zip_with pair gsl hsl)
        ...
        Proof.
          ...
          +
            ...
            (vector_inv_0 hsl), (vector_inv_0 gsl).
            assert (ha : (zip_with (λ '(gi, hi) '(ui, ci), gop (gi ^ ui) (hi ^ opp ci)) 
            (zip_with pair [] [])
          ...
          +
            ...
            destruct (vector_inv_S gsl) as (gslh & gslt & hd).
            subst.
            ...
            ++
              rewrite hd. cbn.
              specialize (ihm _ f' uslt (c - cslh) cslt csr x u usr hslt h hsr 
              gslt g gsr).
        ...
        Theorem construct_or_conversations_simulator_completeness_base : 
          ∀ (n : nat) (f : Fin.t n) (usr : Vector.t F n)
          (gsr hsr : Vector.t G n) (cstt : Vector.t F n),
          gsr[@f] ^ usr[@f] =
          gop
          (zip_with (λ '(gi, hi) '(ui, ci), gop (gi ^ ui) (hi ^ opp ci)) 
          (zip_with pair gsr hsr)
      ...
      Theorem construct_or_conversations_simulator_completeness_generic : 
          ...
          (gsl hsl : t G m) (g h : G) (gsr hsr : t G n)
          (cst : t F (m + S n)),
          (gsl ++ [g] ++ gsr)[@f] ^ (usl ++ u :: usr)[@f] =
          gop
          (zip_with (λ '(gi, hi) '(ui, ci), gop (gi ^ ui) (hi ^ opp ci))
          (zip_with pair (gsl ++ [g] ++ gsr) (hsl ++ h :: hsr))
        ...
        Lemma construct_or_conversations_schnorr_completeness {m n : nat}
          (x : F) (* secret witness *) 
          (gsl hsl : Vector.t G m) (g h : G) (gsr hsr : Vector.t G n) (R : h = g ^ x) : 
          ∀ (uscs : Vector.t F (m + (1 + n) + (m + n))) (c : F),
          generalised_or_accepting_conversations (gsl ++ [g] ++ gsr) (hsl ++ [h] ++ hsr)
          (construct_or_conversations_schnorr x (gsl ++ [g] ++ gsr) 
          (hsl ++ [h] ++ hsr) uscs c) = true.
        Proof.
          intros *. cbn.
          ...
          +
            intros f.
            rewrite dec_true, VectorSpec.splitat_append.
        ...
        Theorem generalised_construct_or_conversations_schnorr_completeness_fin : 
          ...
          (gsl hsl : Vector.t G m) (g h : G) (gsr hsr : Vector.t G n) (x : F),
          m = proj1_sig (Fin.to_nat rindex) ->
          (hsl ++ h :: hsr)[@rindex] = (gsl ++ g :: gsr)[@rindex] ^ x -> h = g ^ x.
        Proof.
          intros * ha hb.
        ...
        Lemma generalised_construct_or_conversations_schnorr_completeness {n : nat} 
          (rindex : Fin.t (2 + n)) (x : F) (gs hs : Vector.t G (2 + n))
          (uscs : Vector.t F ((2 + n) + (1 + n))) (c : F) : 
          hs[@rindex] = gs[@rindex]^x -> 
          generalised_or_accepting_conversations gs hs
          (generalised_construct_or_conversations_schnorr rindex x gs hs uscs c) = true.
        Proof.
          intros * hu.
          ...
          destruct (splitat m₁ ((rew [t G] pfa in gs))) as (gsl & gsrr) eqn:hga.
          eapply append_splitat in hga.
          destruct (vector_inv_S hsrr) as (h & hsr & hc).
          rewrite hc in ha. 
          destruct (vector_inv_S gsrr) as (g & gsr & hw).
          rewrite hw in hga. 
          revert hf.
          subst; cbn. intro hf.
          remember ((rew <- [λ n0 : nat, @sigma_proto F G n0 (S n0) n0] pfa in
          @construct_or_conversations_schnorr m₁ m₂ x (rew [t G] pfa in gs) 
          (rew [t G] pfa in hs)
        ...
        Lemma generalised_construct_or_conversations_simulator_completeness {n : nat}
          (* public values *) (gs hs : Vector.t G (2 + n)) : 
          ∀ (uscs : Vector.t F ((2 + n) + (1 + n))) (c : F),
          generalised_or_accepting_conversations gs hs
          (generalised_construct_or_conversations_simulator gs hs uscs c) = true.
        Proof.
          intros *.
          eapply generalised_or_accepting_conversations_correctness.
          ...
          +
            ...
            destruct (vector_inv_S hsrr) as (h & hsr & hc).
            subst.
            destruct (splitat m₁ gs) as (gsl & gsrr) eqn:hb.
            eapply append_splitat in hb.
            destruct (vector_inv_S gsrr) as (g & gsr & hc).
            subst.
        ...
        Proof.
          ...
          +
            ...
            rewrite Hg; cbn.
            destruct (vector_inv_S gs) as (gsh & gstl & Hhh).
            rewrite (vector_inv_0 gstl) in Hhh.
            rewrite Hhh; cbn.
            specialize (Har Fin.F1).
            specialize (Hbr Fin.F1).
            cbn in * |- *.
            rewrite dec_true, Hg, Hhh in Har, Hbr.
        ...
        Lemma generalised_or_sigma_soundness 
          {m n : nat}
          (gsl hsl : Vector.t G m) 
          (g h : G)
          (gsr hsr : Vector.t G n) :
        ...
        Proof.
          ...
          subst.
          destruct (splitat m₁ gs) as (gsl & gsrr) eqn:hd.
          eapply append_splitat in hd.
          destruct (vector_inv_S gsrr) as (g & gsr & he).
          subst.
        ...
        Lemma or_schnorr_distribution_probability_generic 
          ...
          List.In (trans, pr)
            (Bind l (λ uscs :  Vector.t F (m + (1 + n) + (m + n)),
              ...
              (gsl ++ [g] ++ gsr) (hsl ++ [h] ++ hsr) uscs c))) → pr = 1 / q.
        Proof.
         ...
         Lemma generalised_or_schnorr_distribution_transcript_generic 
          ...
          List.In (trans, pr)
            (Bind l (λ uscs : Vector.t F ((2 + n) + (1 + n)),
              Ret (generalised_construct_or_conversations_schnorr rindex x 
              gs hs uscs c))) → 
          generalised_or_accepting_conversations gs hs trans = true.
        Proof.
          induction l as [|(a, p) l IHl].
          +
            intros * Ha.
        ...
        *)
        ...
        Lemma generalised_or_special_honest_verifier_schnorr_dist 
          ...
          generalised_or_accepting_conversations gs hs a = true ∧ 
        ...
        *)
        ...
        (* The simulator with its length index made explicit; definitionally 
          ...
          the length. *)
        Definition or_simulator_base {N : nat} (gs hs : Vector.t G (S N)) 
          ...
          let commitment := zip_with (fun '(gi, hi) '(ui, ci) => gop (gi^ui) (hi^(opp ci))) 
            (zip_with pair gs hs) comm in
          (commitment; c :: cm; us).

        Lemma or_simulator_base_eq {n : nat} (gs hs : Vector.t G (2 + n)) 
          (uscs : Vector.t F ((2 + n) + (1 + n))) (c : F) :
          generalised_construct_or_conversations_simulator gs hs uscs c = 
          @or_simulator_base (1 + n) gs hs uscs c.
        ...
        Lemma construct_or_shift {n : nat} (rindex : Fin.t (2 + n)) (x : F) 
          (gs hs : Vector.t G (2 + n)) (R : hs[@rindex] = gs[@rindex] ^ x) :
          forall (uscs : Vector.t F ((2 + n) + (1 + n))) (c : F),
          generalised_construct_or_conversations_schnorr rindex x gs hs uscs c =
          generalised_construct_or_conversations_simulator gs hs 
        ...
        Proof.
          ...
          generalize dependent pfa; generalize dependent hs; 
          generalize dependent gs; generalize dependent rindex.
          set (N := (1 + n)%nat) in *.
          clearbody N.
          assert (pfb' : (m₁ + m₂)%nat = N) by (symmetry; exact pfb).
          destruct pfb'.
          rewrite (UIP_nat _ _ pfb eq_refl); cbn [eq_rect_r eq_rect eq_sym].
          intros rindex hd gs hs R pfa hb.
          rewrite !rew_opp_r.
          destruct (splitat m₁ (rew [t G] pfa in gs)) as (gsl & gsrr) eqn:hga.
          apply append_splitat in hga.
          destruct (vector_inv_S gsrr) as (g & gsr & hg); subst gsrr.
          ...
          pose proof (f_equal (fun v => rew <- [t G] pfa in v) hga) as hgs.
          pose proof (f_equal (fun v => rew <- [t G] pfa in v) hha) as hhs.
          cbn beta in hgs, hhs.
          rewrite rew_opp_l in hgs, hhs.
          rewrite hga, hha.
          subst gs hs.
