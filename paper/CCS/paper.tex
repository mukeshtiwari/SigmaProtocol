...
each vote is either 0 (not approved) or 1 (approved).  The frontend functions, shown in Listing ..., realise the ballot-construction pipeline for approval voting as ``encrypt then prove validity.'' The function \texttt{encrypt\_vote} computes the ElGamal ciphertext \((g^r,\; g^m \cdot h^r)\) from generator \(g\), public key \(h\), vote value \(m\), and randomness \(r\). The function \texttt{generate\_enc\_proof} then constructs a two-branch \(\Sigma\)-proof that the ciphertext encrypts either \(0\) or \(1\): it performs a case split with \texttt{Fdec m zero}, places the real witness in the corresponding branch index (\texttt{Fin.F1} when \(m=0\), otherwise \texttt{Fin.FS Fin.F1}), and simulates the other branch. Finally, \texttt{encrypt\_vote\_and\_generate\_enc\_proof} composes both steps and returns the ciphertext together with its proof as a single object for verification. Importantly, this wrapper is total: if \(m \notin \{0,1\}\), it still returns a proof term syntactically, but verification rejects it, matching the later theorems \texttt{vote\_proof\_invalid\_reject} and \texttt{vote\_proof\_invalid\_accept}. More precisely, \texttt{vote\_proof\_invalid\_reject} shows that for invalid votes \(m \neq 0,1\), verification returns false under the usual non-degenerate assumptions. The companion theorem \texttt{vote\_proof\_invalid\_accept} captures the exceptional case: if the prover can predict the verifier challenge (\(c_1 = c\)), an invalid transcript can still be accepted. Together, these results characterise the standard \(\Sigma\)-protocol soundness boundary and explain why unpredictable challenges are essential.
...
\textbf{Homomorphic Tallying}: Once an election finishes, election election authorities tally the results 
using homomorphic aggregation, implemented in \texttt{src/Backend/Tally.v}. The central backend function is \texttt{compute\_final\_count}, which takes the secret key \(x\), randomness vectors for decryption proofs, a proof of key consistency \(g^x = h\), and the list of ballots, and returns dependent data \((vbs, inbs, pt)\) together with a proof term of type \texttt{count (finished ...)}. Internally, it first calls \texttt{compute\_final\_tally}, which processes ballots with the inductive state machine \texttt{count}: rule \texttt{ax} initializes the tally to identity ciphertexts, \texttt{cvalid} accepts and aggregates ballots whose encryption proofs verify, and \texttt{cinvalid} rejects malformed ballots while preserving the tally. From the final encrypted tally \(ms\), the backend computes decrypted group elements \texttt{ds} via \texttt{decrypted\_ballot}, then maps \texttt{discrete\_logarithm\_search} to obtain plaintext tallies \texttt{pt}; lemma \texttt{compute\_final\_count\_aux} (using axiom \texttt{hdiscrete}) proves \(g^{pt_i}=ds_i\) component-wise. In parallel, \texttt{construct\_decryption\_proof\_elgamal\_real\_vector} builds trustee decryption proofs \texttt{pf}, and \texttt{compute\_final\_tally\_aux2} supplies the algebraic relation required to show these proofs are accepting. Finally, rule \texttt{cfinish} combines ballot-partition correctness, plaintext consistency, and decryption-proof validity into a certified finished election state.
...
\textbf{Implementation Details}: The (src/Executable/HeliosVerifiercode/main.ml) 
