...
ensure our library is practically usable. \textcolor{blue}{Finally, we extract OCaml,, Rust, and WebAssembly code from our certified 
Rocq implementation using Peregrine\footnote{https://peregrine-project.github.io/}. We demonstrate the applicability of our framework by implementing a voting client, extracted to WebAssembly from Rocq proofs, and a server, extracted to Rust from Rcoq proofs, for Approval Voting and a complete Helios election verifier that validates IACR elections (2023, 2024), verifying ballot proofs, tallying, and trustee decryption proofs}. To the best of our knowledge,  this is the first formal verification of $\Sigma$-protocols that can be extracted to WebAssembly, Rust, and OCaml from Rocq proofs. Our proofs can be accessed from this \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/README.md}{\textcolor{green}{anonymous repo}}, which contains 30,000 lines of Rocq proofs.
...
Section \ref{coq_theorem} briefly presents the Rocq theorem prover and the MetaRocq infrastructure used for extraction to targets such as WebAssembly and Rust. 
...
for electronic voting in WebAssembly and Rust.
...
artefacts. For example, Rocq has been used to 
...
a smart contract certification framework, etc.
...
\begin{itemize}
  ...
  we have 
  \[ g^r = a * h^c, \quad g^{r'} = a * h^{c'}. \]
  It follows that $h = g^{(r-r')/(c-c')}$, hence we obtain
  the witness $w$ satisfying $h=g^w$ as $w = (r - r')/(c - c')$.
...
later be also instantiated with elliptic curve group. In addition, 
...
protocol, the simulator, and the verification equation in Rocq as showing in 
Listing \ref{schn_nat}. The source code for this section can be accessed from the
\href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Crypto/Sigma.v}{\textcolor{green}{Sigma.v}} file from our anonymous repo.
...
\begin{itemize}
    \item Vector addition: for any vectors \(\mathbf{v}, \mathbf{w} \in V\), their sum, denoted by \(\mathbf{v} + \mathbf{w}\), is also in \(V\).
    \item Scalar multiplication: for any vector \(\mathbf{v} \in V\) and any scalar \(c \in F\), their product, denoted by \(c\mathbf{v}\), is also in \(V\).
\end{itemize}
...
(i) closure under addition: \(\mathbf{u} + \mathbf{v} \in V\), (ii) commutativity of addition: \(\mathbf{u} + \mathbf{v} = \mathbf{v} + \mathbf{u}\), 
(iii)  associativity of addition: \((\mathbf{u} + \mathbf{v}) + \mathbf{w} = \mathbf{u} + (\mathbf{v} + \mathbf{w})\), 
(iv) existence of zero vector: there exists a vector \(\mathbf{0} \in V\) such that \(\mathbf{v} + \mathbf{0} = \mathbf{v}\) for all \(\mathbf{v} \in V\), 
(v) existence of additive inverse: for each vector \(\mathbf{v} \in V\), there exists a vector \(-\mathbf{v} \in V\) such that \(\mathbf{v} + (-\mathbf{v}) = \mathbf{0}\),
(vi) closure under scalar multiplication: \(c\mathbf{v} \in V\), 
(vii) distributive properties: \(c(\mathbf{u} + \mathbf{v}) = c\mathbf{u} + c\mathbf{v}\) and \((c + d)\mathbf{v} = c\mathbf{v} + d\mathbf{v}\), 
and (viii) compatibility with field multiplication: \(c(d\mathbf{v}) = (cd)\mathbf{v}\) and \(1\mathbf{v} = \mathbf{v}\), where \(1\) denotes the multiplicative identity in \(F\). In our setting we instantiate the set of vectors with $G$ and field with $F$. 
...
distribution in Rocq that we introduce in the next section.
...
encode the distribution, shown in Listing \ref{prob_def}, as a list of tuples where the 
...
to encode any kind of distribution. We prove that the \texttt{dist} is a 
monad \cite{erwig2006functional} by 
giving it a suitable definitions of \texttt{Ret} and 
\texttt{Bind}. Making it an instance of monad allows us 
...
zero-knowledge of $\Sigma$-protocol, and therefore we define a function 
\texttt{\seqsplit{uniform\_with\_replacement}} to generate uniform distributions, shown in Listing \ref{prob_def}.
The relevant source code can be accessed from \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Probability/Prob.v}{\textcolor{green}{Prob.v}} and \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Probability/Distr.v}{\textcolor{green}{Distr.v}}.
...
to construct their respective distributions on the sigma protocol.
...
The Okamoto protocol follows the standard three-move $\Sigma$-protocol structure. Given public generators $\mathbf{g} = (g_1, g_2, \ldots, g_n)$, a public value $h$, and secret witnesses $\mathbf{x} = (x_1, x_2, \ldots, x_n)$ satisfying $h = \prod_{i=1}^{n} g_i^{x_i}$, the prover begins by selecting random values from the underlying field $\mathbf{u} = (u_1, u_2, \ldots, u_n)$ and computing the commitment $t = \prod_{i=1}^{n} g_i^{u_i}$, which is sent to the verifier. Next, the verifier responds with a random challenge $c$ from the field. Finally, the prover computes responses $r_i = u_i + c \cdot x_i$ for each $i \in {1, \ldots, n}$ and sends the vector $\mathbf{r} = (r_1, r_2, \ldots, r_n)$ to the verifier for verification. The verifier accepts the transcript $(t, c, \mathbf{r})$ if and only if 
...
Our Rocq formalisation implements the generalised Okamoto protocol for $n \geq 2$ generators and can be accessed in the file \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Crypto/Okamoto.v}{\textcolor{green}{Okamoto.v}}. The key functions include: \texttt{\seqsplit{generalised\_okamoto\_commitment}} (computes $\prod_{i=1}^{n} g_i^{u_i}$), \texttt{\seqsplit{generalised\_okamoto\_response}} (computes responses $r_i = u_i + c \cdot x_i$), and 
...
soundness of the Neq protocol that uses Okamoto as a building block; and (iii) \textbf{special honest-verifier zero-knowledge}: 
...
\prod_{i=1}^{n} g_i^{x_{2,i}}$ are computationally indistinguishable. The proof \texttt{\seqsplit{generalised\_okamoto\_witness\_indistinguishable}}, shown in Listing \ref{wit_ind}, relies on two 
...
the prover must demonstrate that $x_i \neq x_j$ without revealing the values. Our implementation works as follows: the protocol begins with the commitment phase, in which the prover generates randomness for two components. For the And component, the prover uses $2+n$ random values to create commitments $t_i = g_i^{u_i}$ for each $i \in \{1, \ldots, 2+n\}$, proving knowledge of all discrete logarithms. For the Okamoto component, the prover generates an Okamoto commitment for each pair $(i, j)$ with $i < j$ using generators $(g_i \cdot g_j, h_i \cdot h_j)$ and specially constructed witnesses $\left(x_i \cdot (x_i - x_j)^{-1}, (x_j - x_i)^{-1}\right)$ that encode the inequality constraint, resulting in $\binom{2+n}{2} = (2+n)(1+n)/2$ such commitments. The combined commitment vector, of dimension $(2+n) + \binom{2+n}{2}$, is then sent to the verifier. In the challenge phase, the verifier responds with a random challenge $c$ from the underlying field. Finally, in the response phase, the prover computes two sets of responses: the And responses $r_i = u_i + c \cdot x_i$ for each $i$ (as in the standard And composition), and the Okamoto responses—a two-dimensional response for each pair $(i, j)$ using the transformed witnesses and randomness, yielding $(2+n)(1+n)$ field elements in total. This results in a response vector of dimension $(2+n) + (2+n)(1+n) = (2+n)^2$, which is sent to the verifier.


The verifier accepts if both sub-protocol verifications succeed: (i) the And verifier confirms that 
each $g_i^{r_i} = t_i \cdot h_i^c$, and (ii) for each pair $(i, j)$, the Okamoto verifier confirms that 
the relation $(g_i \cdot g_j)^{r_{i,1}} \cdot (h_i \cdot h_j)^{r_{i,2}} = t_{i,j} \cdot g_j^c$ holds, 
...
$\left(x_i \cdot (x_i - x_j)^{-1}, (x_j - x_i)^{-1}\right)$ satisfy $(g_i \cdot g_j)^{x_i \cdot (x_i - x_j)^{-1}} \cdot (h_i \cdot h_j)^{(x_j - x_i)^{-1}} = g_j$ where we use $h_i = g_i^{x_i}$, $h_j = g_j^{x_j}$, and $(x_j - x_i)^{-1} = -(x_i - x_j)^{-1}$.

Our Rocq formalisation, can be accessed in the file \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Crypto/NeqSigma.v}{\textcolor{green}{NeqSigma.v}}, proves the three standard $\Sigma$-protocol properties: (i) \textbf{completeness}: 
When the prover knows distinct witnesses $x_1, \ldots, x_{2+n}$ satisfying $g_i^{x_i} = h_i$ and $x_i \neq x_j$ 
for all $i \neq j$, the verification always succeeds. The proof combines completeness results from both the And composition completenes and the Okamoto protocol completeness for each pair; (ii) \textbf{special honest-verifier zero-knowledge}: 
...
group of integers modulo a large prime p), shown in Listing \ref{group_def}, in the Rocq theorem prover. 
...
$k = 2$, these primes are called Sophie Germain primes). Finally, we encode the neutral element (\texttt{one}), the group multiplication (\texttt{\seqsplit{mul\_schnorr\_group}}), 
...
the scalar mulitplication \texttt{pow} of 
...
The source code for this section can be accessed in the file \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Utility/Zpstar.v}{\textcolor{green}{Zpstar.v}}.
...
proof. A complete ballot is thus a vector of $n$ such tuples. We extract all frontend functions into WebAssembly. The related Rocq file can be accessed from the \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Frontend/Approval.v}{\textcolor{green}{Approval.v}} file and the corresponding WebAssembly file from \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Wasm/approval.wasm}{\textcolor{green}{approval.wasm}}.
...
\textbf{Verification Rules}: The election verification process follows a very similar approach described in the 
previous section. We have an inductive datatype \texttt{count}, index by \texttt{state}, defined in \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Backend/HeliosTally.v}{\textcolor{green}{HeliosTally.v}}. The \texttt{count} and \texttt{state} are similar to previously defined, except the fourth constructor \texttt{finish} is 
...
which is the final discrete log equality check; (iii) Proof of knowledge (POK) checks: Each tallier has published a Schnorr proof of knowledge of their 
...
The function \texttt{compute\_final\_count} is the top-level certified
verifier. Its type states that for \emph{any} list of ballots $\mathit{bs}$,
any vector of $m{+}1$ talliers $\mathit{ts}$, and any published plaintext
...
certificate of the entire end-to-end verification.
%
Concretely, \texttt{compute\_final\_count} first calls
\texttt{compute\_final\_tally} to process each ballot, accepting those
whose ElGamal encryption proofs verify and rejecting the rest, while
homomorphically aggregating valid ciphertexts into the encrypted tally
$\mathit{ms}$. It then applies the \texttt{finish} rule, which simultaneously
(i) checks that the supplied decryption factors $d_i = \alpha^{s_i}$ are
correctly attested by Chaum-Pedersen proofs, (ii) checks the plaintext tally
by verifying $g^{\mathit{pt}_j} = \mathit{ds}_j$ for every candidate $j$,
(iii) verifies each tallier's Schnorr proof of knowledge of their secret
key, and (iv) confirms that the product of all tallier public keys equals the
election public key $h$. The conjunction of these four Boolean outcomes
is $\mathit{bfinal}$. Because the entire computation is carried out inside
Rocq's kernel, $\mathit{bfinal} = \texttt{true}$ is a \emph{proof} rather
than merely a runtime claim.


\textbf{Concrete Instantiation via \texttt{compute\_final\_count\_ins}}:
...
The function \texttt{compute\_final\_count\_ins}, defined in \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Examples/HeliosTallyIns.v}{\textcolor{green}{HeliosTallyIns.v}}, fixes every one of these
...
the tallier vector $\mathit{ts}$, and the published plaintext tally
$\mathit{pt}$.
...
$\texttt{count}~(\texttt{finished}~\mathit{bs}~\mathit{vbs}~\mathit{inbs}~\mathit{ts}~\mathit{pt}~\mathit{bfinal})$,
which is a machine-checked certificate that the election tally is correct.

\textbf{Primality of $p$ and $q$}:
The two large primes used in the Helios parameters are the 256-bit prime~$q$
and the 2048-bit safe prime~$p = kq + 1$.
Their primality is established inside Rocq using the
\textsc{Coqprime}~\cite{thery2007primality} library, which implements a
\emph{reflected} Pocklington–Lucas primality test: a structured primality
certificate is supplied as a Rocq term and then verified by a single
\texttt{vm\_cast\_no\_check} kernel call, keeping the trusted computing base
small while avoiding expensive computation in the proof engine.

For $q$, the proof in \texttt{primeQ.v} uses a standard
\texttt{Pock\_certif} certificate.  The Pocklington criterion requires
that $q - 1$ can be factored with each prime factor itself certified;
here $q - 1$ is expressed as a product of small primes
$(2^2 \cdot 3^3 \cdot 7 \cdot 23 \cdot 97 \cdot 4{,}223{,}919{,}769 \cdot 6{,}350{,}820{,}103)$,
and each factor that is not immediately known prime is recursively handled by
a nested \texttt{Pock\_certif}.

For $p$, the proof in \texttt{primeP.v} uses an \texttt{Ell\_certif}
(elliptic-curve primality certificate) because the size of $p$ (2048 bits)
makes a purely Pocklington-style factorisation of $p - 1$ impractical.
The elliptic-curve certificate encodes the witness for the Goldwasser-Kilian
criterion, and the factorisation chain is built up recursively via a
sequence of \texttt{Pock\_certif} and \texttt{Proof\_certif} leaves.
In both cases, the certificate is checked by \texttt{vm\_cast\_no\_check},
which delegates the boolean equality to Rocq's efficient virtual-machine
evaluator rather than to the tactic engine.

\textbf{Execution in \texttt{main.ml}}:
The extracted OCaml executable in \texttt{main.ml} ties together the parser
and the certified verifier into a single end-to-end pipeline.
The program reads election data from standard input, which is structured
in three sections separated by semicolons: the list of voter ballots
(\texttt{bs}), the vector of trustees (\texttt{us}), and the published
plaintext tally (\texttt{rs}).
The parser, generated from the Menhir grammar in \texttt{parser.mly},
reconstructs each ballot as a tuple containing the cast timestamp,
a vector of ElGamal ciphertexts $(\alpha_i, \beta_i)$, a vector of Or-sigma
proofs (one per candidate, certifying that the plaintext is $0$ or $1$),
...
and the trustee's public key $(g, p, q, y_i)$.
The challenge for each Or-sigma proof is reconstructed as the triple
$(c_0 + c_1 \bmod q,\; c_0,\; c_1)$, matching the commitment structure
expected by the Rocq formalisation.

Once parsing is complete, \texttt{compute\_final\_count\_ins} is invoked
...
The program then prints the full certificate trace via \texttt{print\_count},
which recursively deconstructs the \texttt{count} inductive, displaying for
each ballot whether it was accepted or rejected and, at the \texttt{finish}
node, reporting the four individual Boolean checks---decryption proof
validity, plaintext consistency, proof-of-knowledge validity, and public-key
product consistency---together with the conjunction \texttt{bfinal}.
...
produce a zero-knowledge proof to ensure the correctness of the shuffle \cite{10.1007/978-3-642-02620-1_28,10.1007/978-3-642-29011-4_17}. Another possibility is to formalise Bulletproofs \cite{bunz2018bulletproofs}; this would allow us to provide compact range proofs for ballot validity (e.g., proving that a vote is $0$ or $1$), and could also be adapted to construct verifiable shuffles with logarithmic proof size (we have encoded the inner-product protocol but have not yet proved any security properties about our encoding). Another promising direction is a domain-specific language (DSL) for sigma protocols embedded in Rocq. The \texttt{\seqsplit{sigma-compiler}}~\cite{sigma-compiler} macro for Rust allows a protocol designer to write a high-level, declarative specification—lists of public parameters, secret witnesses, and logical statements—and automatically synthesises the corresponding prover and verifier functions. While this eliminates many manual implementation errors, the generated code itself is not formally verified. By embedding a similar DSL within Rocq, one could obtain a correct-by-construction pipeline: the same specification that is used to generate executable OCaml or Rust code could also serve as the input to a formalisation that proves, inside Rocq, that the resulting proof satisfies completeness, special soundness, and honest-verifier zero-knowledge. Given our current infrastructure, it would be fairly straightforward to  encode these algorithms and prove their security properties. 
   ...
   work about formalising random oracle in the Rocq theorem 
  ...
  In this paper, we present a certified implementation that the voting community can readily adopt to encode their protocol with all security guarantees. We demonstrate its usefulness by encoding a Helios verifier and a client–server system for Approval voting. Moreover, our formalisation extracts to OCaml, WebAssembly, and Rust without redundant effort.
