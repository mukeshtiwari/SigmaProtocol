...
\begin{abstract}
  ...
  critical shortcomings: (i) the same code is written twice: once for the front-end (JavaScript/TypeScript)
  and once for the back-end (Python/OCaml/Java), which is error-prone because the front-end and back-end codes are often written by different developers, leading to inconsistencies and bugs, and (ii) none of these implementations provide formal security guarantees. 
...
\textcolor{red}{How to make this flow natural between the previous and the next paragraph?}
...
\begin{itemize}
  ...
  \item \textbf{special honest-verifier zero-knowledge proof:}
   ...
   conversation--involving the witness $w$--and the distribution of transcripts output by a
   simulator--without the witness $w$--are identical. We can see this algebraically: both
...
Using these definitions, we prove that the two distributions coincide in the $special\_honest\_verifier\_zkp$ theorem. Unlike the standard informal presentation pervasive throughout the cryptography literature, these distributions are not syntactically identical but they coincide modulo $accepting\_conversation$. We therefore apply $accepting\_conversation$to the first (transcript) component of each pair in both distributions, obtaining the boolean value $true$ in all cases. After this transformation, the resulting distributions are identical.
...
As we stated previously that we have also formalised methods for combining $n$ \textit{And}, \textit{Or}, \textit{Eq}, \textit{Parallel}, and \textit{Neq} Schnorr statements, where proving the special honest-verifier zero-knowledge property is more challenging than for a single Schnorr statement. One major challenge is drawing $n$ random elements from a uniform distribution. We therfore define the function $repeat\_dist\_ntimes\_vector$---shown in Listing~\ref{dist_comp}---which takes a distribution $d$ and a number $n$ and returns a distribution over vectors of length $n$, encoded as $dist\,({Vector.t}\,A\,n)$. Note that this is a generic function that can be instantiated with any distribution. In cryptography, however, we are primarily concerned with the uniform distribution; in the theorem $uniform\_probability\_multidraw\_prob$, we instantiate the distribution with $uniform\_with\_replacement\;lf\;Hlf$ and prove that its probability is $1/|lf|^n$. We use this function consistently throughout the formalisation. As an example, we illustrate a case of the parallel composition in which \(n\) statements are combined, requiring a vector of \(n\) random field elements to establish the zero-knowledge property, as shown in $parallel\_schnorr\_distribution$.
...
src/Crypto/Okamoto.v. The key functions include: $generalised\_okamoto\_commitment$ (computes $\prod_{i=1}^{n} g_i^{u_i}$), 
$generalised\_okamoto\_response$ (computes responses $r_i = u_i + c \cdot x_i$), and 
$generalised\_okamoto\_accepting\_conversation$ (verifies the product equality condition). 
We also provide specialised instances for the base case of exactly two generators ($n=2$), which is heavily used in the Neq composition. The formalisation proves all three security properties:
\begin{itemize}
\item \textbf{Completeness}: 
...
\prod_{i=1}^{n} g_i^{x_{2,i}}$ are computationally indistinguishable. The proof, shown in Listing \ref{wit_ind}, 
$generalised\_okamoto\_witness\_indistinguishable$ relies on two 
...
each vote is either 0 (not approved) or 1 (approved).  The frontend functions, shown in Listing \ref{frontend_fun}, realise the ballot-construction pipeline for approval voting. The function \texttt{encrypt\_vote} computes the ElGamal ciphertext \((g^r,\; g^m \cdot h^r)\) from generator \(g\), public key \(h\), vote value \(m\), and randomness \(r\). The function \texttt{generate\_enc\_proof} then constructs a two-branch \(\Sigma\)-proof that the ciphertext encrypts either zero (\(0\)) or 
a non-zero value: it performs a case split with \texttt{Fdec m zero}, places the real witness in the corresponding branch index (\texttt{Fin.F1} when \(m=0\), otherwise \texttt{Fin.FS Fin.F1}), and simulates the other branch. Finally, \texttt{encrypt\_vote\_and\_generate\_enc\_proof} composes both steps and returns the ciphertext together with its proof as a single object for verification. Ideally, for the non-zero case the voter should encrypt \(1\) but it runs on the voter's computer so the voter can encrypt any value and produce a zero-knowledge proof but the verification function running at the backend will reject the proof, except for some negligible probability.  More precisely, \texttt{vote\_proof\_invalid\_reject} shows that for invalid votes \(m \neq 0,1\), verification returns false under the usual non-degenerate assumptions. The companion theorem \texttt{vote\_proof\_invalid\_accept} captures the exceptional case: if the prover can predict the verifier challenge (\(c_1 = c\)), an invalid transcript can still be accepted. Together, these results characterise the standard \(\Sigma\)-protocol soundness boundary and explain why unpredictable challenges are essential \cite{10.1007/978-3-642-34961-4_38,dao2023weak}
...
Definition generate_enc_proof 
  (g h : G) (r : F)  (m : F) (uscs : Vector.t F 3) (cp : G * G) (c : F) : 
  @Sigma.sigma_proto F (G * G) 2 3 2 :=
...
Finally, the function \texttt{encrypt\_ballot\_and\_generate\_enc\_proof} extends vote encryption to the entire ballot by vectorizing across all $n$ candidates. Each candidate 
produces a tuple $(C_i, \pi_i)$ where $C_i$ is the encrypted vote and $\pi_i$ is the corresponding Or-$\Sigma$ 
proof. A complete ballot is thus a vector of $n$ such tuples.
...
\textbf{Homomorphic Tallying}: Once voting ends, counting is specified as an inductive state machine $count$, parameterised 
over another inductive datatype $state$, defined in 
\texttt{src/Backend/Tally.v}. The $state$ datatype has two constructors, $partial$ and $finished$, that signifies that the counting of ballots are still going or 
finished. More precisely, \texttt{partial us vbs inbs ms}: the computation is in progress, with
all seen ballots \texttt{us}, valid ballots \texttt{vbs}, invalid ballots \texttt{inbs}, and running encrypted tally
\texttt{ms}, and \texttt{finished us vbs inbs pt}: the computation is finalised and returns plaintext tally vector \texttt{pt}.

The judgment \texttt{count : state -> Type} defines legal counting traces through four rules: (i) \textbf{\texttt{ax}} (initialization): starts from empty ballot lists and an identity tally vector \texttt{ms = \(1_G,1_G\),\ldots,\(1_G,1_G\)}. This means no vote has contributed yet; (ii) \textbf{\texttt{cvalid}} (accept valid ballot): if a new ballot \texttt{u} satisfies \texttt{verify\_encryption\_ballot\_proof ... u = true}, then \texttt{u} is moved into \texttt{vbs}, and the encrypted tally is
updated homomorphically:  \texttt{nms = mul\_encrypted\_ballots ms (map fst u)}. Intuitively, ciphertext components are multiplied coordinate-wise, so exponents (hence votes) add.; (iii) \textbf{\texttt{cinvalid}} (reject invalid ballot): if \texttt{verify\_encryption\_ballot\_proof ... u = false}, then \texttt{u} is appended to \texttt{inbs}, while \texttt{ms} remains unchanged; and (iv) \textbf{\texttt{cfinish}} (finalisation): after all ballots are processed, decryption outputs \texttt{ds}
and decryption proofs \texttt{pf} are checked. The rule requires  pointwise relation \(g^{pt_i}=ds_i\), i.e., plaintext counts are discrete logs of decrypted group elements and acceptance of the vector decryption proof \texttt{decryption\_proof\_accepting\_conversations\_vector ... ms ds pf = true}. Only then the system transitions to \texttt{finished}.


The executable function \texttt{compute\_final\_tally} implements the first phase (classification and encrypted
aggregation) by recursion on the ballot list. For each ballot, it branches on
\texttt{verify\_encryption\_ballot\_proof}: the true branch applies \texttt{cvalid}, the false branch applies
\texttt{cinvalid}. It returns \((vbs,inbs,ms)\) together with the invariant
\texttt{Permutation bs (vbs ++ inbs)}, ensuring that every input ballot is accounted for exactly once. Finally, he function \texttt{compute\_final\_count} then derives the final plaintext tally by decrypting aggregate ciphertexts with secret key $x$ to obtain \texttt{ds}, computing \texttt{pt} by pointwise discrete-log search relative to generator $g$, and conclude \texttt{count (finished bs vbs inbs pt)} via \texttt{cfinish}. Hence, the formal counting pipeline is: \emph{verify each ballot} $\to$ \emph{homomorphically aggregate valid ciphertexts} $\to$ \emph{verify decryption} $\to$ \emph{extract plaintext counts}, with explicit proof obligations at each step.
