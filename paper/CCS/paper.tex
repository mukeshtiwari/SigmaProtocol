...
\begin{itemize}
  ...
  \item Explain the rationale of VectorSpace (it avoids the misuse of API and cite Thomas paper)
  \item Explain polynomial-time extractor 
  \item Explain the Helios Verifier case study but highlight that it is more general and can be used in other protocols as well. 
  \item 
\end{itemize}
...
\textcolor{blue}{Is this flow good between the previous and the next paragraph?}
...
these two distributions are two different transcripts but 
an accepting one\textcolor{red}{Fix: make it nicer}. For both distributions, 
...
\subsection{OR Composition of n Schnorr Protocol}
...
the prover convinces the verifier that they know at least one witness $x_k$ such that $h_k = g_k^{x_k}$ for some $k \in \{1, \ldots, n\}$, 
...
Our formalization implements this protocol generically for $n \geq 2$ statements. The prover, knowing the witness 
for the $k$-th relation (where $k$ is specified as a finite index \texttt{rindex : Fin.t (2+n)}), 
proceeds as follows:
\begin{enumerate}
\item \textbf{Commitment phase}: The prover chooses random values: (i) $u_k$ for the real commitment, 
and (ii) $u_i, c_i$ for each simulated statement $i \neq k$. The prover computes:
\begin{itemize}
\item Real commitment (for statement $k$): $t_k = g_k^{u_k}$
\item Simulated commitments (for $i \neq k$): $t_i = g_i^{u_i} \cdot h_i^{-c_i}$
\end{itemize}
The vector of commitments $(t_1, \ldots, t_n)$ is sent to the verifier.

\item \textbf{Challenge}: The verifier sends a random challenge $c$.

\item \textbf{Response}: The prover computes:
\begin{itemize}
\item $c_k = c - \sum_{i \neq k} c_i$ (to satisfy the linear constraint)
\item $s_k = u_k + c_k \cdot x_k$ (honest response for the real statement)
\item For $i \neq k$, responses $s_i$ were already chosen during commitment
\end{itemize}
The prover sends $(s_1, \ldots, s_n)$ and $(c_1, \ldots, c_n)$.
\end{enumerate}

The verifier accepts if: (i) $c = \sum_{i=1}^{n} c_i$, and (ii) for each $i$, $g_i^{s_i} = t_i \cdot h_i^{c_i}$.

In our Rocq formalization, we implement two versions: \texttt{OrSigma.v} restricts all statements to share 
a common generator $g$ (i.e., relations of the form $g^{x_i} = h_i$), 
while \texttt{OrSigmaGen.v} generalizes to allow different generators for each statement 
(relations $g_i^{x_i} = h_i$). Both versions prove completeness, special soundness (with an extractor 
that recovers a witness given $n$ accepting transcripts with the same commitments but different challenge 
distributions), and special honest-verifier zero-knowledge by showing that the distribution 
of real transcripts (involving the witness) is identical to the simulator distribution (without the witness).


\subsection{Okamoto Protocol}

The Okamoto protocol~\cite{okamoto1992provably} generalizes the Schnorr protocol to support multiple generators, 
enabling proofs of knowledge for linear relations over discrete logarithms. While the Schnorr protocol proves 
...
The protocol follows the standard three-move $\Sigma$-protocol structure. Given public generators 
$\mathbf{g} = (g_1, g_2, \ldots, g_n)$, public value $h$, and secret witnesses 
$\mathbf{x} = (x_1, x_2, \ldots, x_n)$ satisfying $h = \prod_{i=1}^{n} g_i^{x_i}$, the prover and verifier 
interact as follows:

\begin{enumerate}
\item \textbf{Commitment}: The prover chooses random values $\mathbf{u} = (u_1, u_2, \ldots, u_n)$ from the 
underlying field and computes the commitment $t = \prod_{i=1}^{n} g_i^{u_i}$, which is sent to the verifier.

\item \textbf{Challenge}: The verifier sends a random challenge $c$ from the field.

\item \textbf{Response}: The prover computes responses $r_i = u_i + c \cdot x_i$ for each $i \in \{1, \ldots, n\}$ 
and sends $\mathbf{r} = (r_1, r_2, \ldots, r_n)$ to the verifier.
\end{enumerate}
...
Our Rocq formalization implements the generalized Okamoto protocol for $n \geq 2$ generators in 
[Okamoto.v](src/Crypto/Okamoto.v). The key functions include: 
...
We also provide specialized instances: \texttt{okamoto\_commitment} and \texttt{okamoto\_response} 
for the base case of exactly two generators ($n=2$), which is heavily used in the Neq composition.

The formalization proves all three security properties:
\begin{itemize}
\item \textbf{Completeness} (\texttt{generalised\_okamoto\_real\_accepting\_conversation}): 
...
\item \textbf{Special soundness} (\texttt{generalised\_okamoto\_real\_special\_soundenss}): 
...
(\texttt{generalised\_okamoto\_special\_honest\_verifier\_zkp}): 
...
Beyond the standard $\Sigma$-protocol properties, our formalization includes witness indistinguishability 
...
randomness between the two witness spaces, along with the homomorphic property 
\texttt{generalised\_okamoto\_commitment\_homomorphic} showing that 
$\prod_{i=1}^{n} g_i^{u_i - v_i} = \left(\prod_{i=1}^{n} g_i^{u_i}\right) \cdot 
\left(\prod_{i=1}^{n} g_i^{-v_i}\right)$.


\subsection{Neq Composition of n Schnorr Protocol}
...
in [NeqSigma.v](src/Crypto/NeqSigma.v) works as follows:

\begin{enumerate}
\item \textbf{Commitment phase}: The prover generates randomness for two components:
\begin{itemize}
\item \textbf{And component}: Uses $n$ random values to create commitments 
$t_i = g^{u_i}$ for each $i \in \{1, \ldots, n\}$, proving knowledge of all discrete logarithms.
\item \textbf{Okamoto component}: For each pair $(i, j)$ with $i < j$, the prover generates 
an Okamoto commitment using generators $(g \cdot h_i, h_j)$ with specially constructed witnesses 
$\left(x_i \cdot (x_i - x_j)^{-1}, (x_j - x_i)^{-1}\right)$ that encode the inequality constraint. 
This yields $\binom{n}{2} = n(n-1)/2$ Okamoto commitments.
\end{itemize}
The total commitment vector has dimension $n + \binom{n}{2}$ and is sent to the verifier.

\item \textbf{Challenge}: The verifier sends a random challenge $c$ from the underlying field.

\item \textbf{Response}: The prover computes:
\begin{itemize}
\item \textbf{And responses}: For each $i$, compute $r_i = u_i + c \cdot x_i$ as in the standard 
And composition.
\item \textbf{Okamoto responses}: For each pair $(i, j)$, compute the two-dimensional Okamoto response 
using the transformed witnesses and randomness, yielding $n(n-1)$ field elements in total.
\end{itemize}
The response vector has dimension $n + n(n-1) = n^2$ and is sent to the verifier.
\end{enumerate}
...
The correctness of the Okamoto component for inequality relies on the algebraic identity: if $x_i = x_j$, 
...
\[
(g \cdot h_i)^{x_i \cdot (x_i - x_j)^{-1}} \cdot h_j^{(x_j - x_i)^{-1}} = 
g^{x_i \cdot (x_i - x_j)^{-1}} \cdot h_i^{x_i \cdot (x_i - x_j)^{-1}} \cdot h_j^{(x_j - x_i)^{-1}} = g
\]
after algebraic simplification using $h_i = g^{x_i}$ and $h_j = g^{x_j}$.

Our Rocq formalization proves the three standard $\Sigma$-protocol properties:

\begin{itemize}
\item \textbf{Completeness} (\texttt{generalised\_neq\_real\_transcript\_accepting\_conversations}): 
When the prover knows distinct witnesses $x_1, \ldots, x_n$ satisfying $g^{x_i} = h_i$ and $x_i \neq x_j$ 
for all $i \neq j$, the verification always succeeds. The proof combines completeness results from both 
the And composition (via \texttt{construct\_and\_conversations\_schnorr\_completeness}) and the Okamoto 
protocol (via \texttt{generalised\_okamoto\_real\_accepting\_conversation}) for each pair.

\item \textbf{Special soundness} (\texttt{generalised\_neq\_accepting\_conversations\_soundenss}): 
...
However, proving that the extracted witnesses are necessarily distinct requires an additional assumption 
about generator independence. Our formalization proves an unconditional dichotomy: either the extracted 
witnesses are pairwise distinct, \emph{or} there exists a non-trivial discrete logarithm relation between 
generators (e.g., $g_j = g_i^\alpha$ for some $\alpha$, or one generator is the identity). This captures 
special soundness without assuming independence explicitly, leveraging decidability of the relevant 
propositions to convert the implication into a constructive disjunction.

\item \textbf{Special honest-verifier zero-knowledge} 
(\texttt{generalised\_neq\_special\_honest\_verifier\_zkp}): 
...
using our uniform probability distribution framework (\texttt{uniform\_probability\_multidraw\_prob}).
\end{itemize}

The implementation requires randomness of dimension $n + n(n-1) = n^2$ and produces proof transcripts 
of size $O(n^2)$, reflecting the quadratic nature of pairwise inequality constraints. The key functions 
include: \texttt{generalised\_construct\_neq\_commitment} (constructs combined And and Okamoto commitments), 
\texttt{generalised\_construct\_neq\_response} (computes responses by splitting into And and Okamoto parts), 
and \texttt{generalised\_neq\_accepting\_conversations} (verifies both sub-protocol checks using 
\texttt{generalised\_and\_accepting\_conversations} and \texttt{okamoto\_accepting\_conversation} 
for each pair).
...
over Pedersen commitments~\cite{pedersen1991non}, which are information-theoretically hiding and 
binding commitments commonly used in cryptographic constructions. Formally, given generators $g$ 
...
voting systems with decentralized tallying, election authorities may commit to their votes using 
...
The protocol follows the standard three-move $\Sigma$-protocol structure. For $n$ commitments with 
a single linear constraint, we require $2n$ fresh randomness values (denoted $u_1, \ldots, u_n$ and 
$w_1, \ldots, w_n$) and produce $(n+1)$ commitment group elements and $2n$ response field elements:

\begin{enumerate}
\item \textbf{Commitment phase}: The prover samples fresh randomness $u_1, \ldots, u_n, w_1, \ldots, w_n$ 
and computes:
\begin{itemize}
\item For each $i \in \{1, \ldots, n\}$: $t_i = g^{u_i} \cdot h^{w_i}$ (commitment mimicking the structure of $C_i$)
\item $t_{n+1} = g^{\sum_{i=1}^{n} \alpha_i \cdot u_i}$ (commitment to the linear combination of randomness)
\end{itemize}
The vector of commitments $(t_1, \ldots, t_n, t_{n+1})$ is sent to the verifier.

\item \textbf{Challenge}: The verifier sends a random challenge $c$ from the field.

\item \textbf{Response}: The prover computes:
\begin{itemize}
\item For each $i$: $s_{1,i} = u_i + c \cdot v_i$ and $s_{2,i} = w_i + c \cdot r_i$
\end{itemize}
The response vectors $(s_{1,1}, \ldots, s_{1,n})$ and $(s_{2,1}, \ldots, s_{2,n})$ are sent.
\end{enumerate}

The verifier accepts the transcript if both verification equations hold:
\begin{enumerate}
\item For all $i$: $g^{s_{1,i}} \cdot h^{s_{2,i}} = t_i \cdot C_i^c$ (each Pedersen commitment verifies)
\item $g^{\sum_{i=1}^{n} \alpha_i \cdot s_{1,i}} = t_{n+1} \cdot g^{c \cdot z}$ (linear constraint verifies)
\end{enumerate}
...
Our Rocq formalization in [PedLinearRel.v](src/Crypto/PedLinearRel.v) implements this protocol 
with generic support for arbitrary $n \geq 2$ commitments and linear constraints. The key functions are:
\begin{itemize}
\item \texttt{pedersen\_commitment\_vector}: Constructs the vector of Pedersen commitments 
from secret values and randomness
\item \texttt{construct\_pedersen\_linear\_relation\_commitment}: Computes the two components 
$(t_1, \ldots, t_n)$ and $t_{n+1}$
\item \texttt{construct\_pedersen\_linear\_relation\_generalised\_real\_proof}: Generates a real proof 
given honest witnesses
\item \texttt{verify\_pedersen\_linear\_relation\_generalised\_proof}: Implements both verification 
equations (\texttt{$ret_1$} and \texttt{$ret_2$})
\end{itemize}

The formalization proves all three security properties:

\begin{itemize}
\item \textbf{Completeness} (\texttt{pedersen\_linear\_relation\_completeness}): 
When the prover knows values $(v_1, r_1), \ldots, (v_n, r_n)$ satisfying both $C_i = g^{v_i} \cdot h^{r_i}$ 
and $z = \sum_i \alpha_i \cdot v_i$, the protocol always verifies successfully. The proof uses the 
helper lemmas \texttt{fold\_right\_zip} (for distributing the challenge over vector operations) and 
the vector space properties of scalar multiplication.

\item \textbf{Simulator completeness} (\texttt{pedersen\_linear\_relation\_simulator\_completeness}): 
Even without access to the witness values $(v_i, r_i)$ and the linear combination sum $z$, 
the simulator can construct accepting transcripts. Given public commitments $C_i$, the simulator 
works backwards: it randomly chooses responses $(s_{1,i}, s_{2,i})$ and challenge $c$, then computes:
\begin{itemize}
\item Fake commitments: $t_i = g^{s_{1,i}} \cdot h^{s_{2,i}} \cdot C_i^{-c}$
\item Fake linear commitment: $t_{n+1} = g^{\sum_i \alpha_i \cdot s_{1,i}} \cdot g^{-c \cdot z}$
\end{itemize}
This ensures the verification equations are satisfied without any knowledge of the actual witness values.

\item \textbf{Special soundness} (\texttt{pedersen\_linear\_relation\_special\_soundness}): 
From two accepting transcripts with the same commitments but different challenges $c_1 \neq c_2$, 
...
For the linear constraint, the formalization proves a dichotomy: either $z = \sum_i \alpha_i \cdot v_i$ holds, 
...
\item \textbf{Special honest-verifier zero-knowledge} 
(\texttt{generalised\_pedersen\_special\_honest\_verifier\_zkp}): 
...
support for multiple candidates, resulting in fairer representation of voter preferences. Our formalization 
...
and phantom ballots. Our formalization proves completeness of Or-$\Sigma$ proofs and the correctness 
of homomorphic aggregation, confirming that the approval voting system is sound and complete.

\subsection{Helios Verifier}

To demonstrate the practical applicability of our formalization, we have developed a certified verifier for 
...
ciphertext aggregation, and decryption verification using the formal proofs embedded in our Rocq formalization.
...
Or-Σ composition with two disjunctions (proving encryption of 0 or 1 for approval voting). If the proof 
...
\begin{itemize}
\item Ballot processing follows the rules of And-Σ composition (verifying multiple disjunctions)
