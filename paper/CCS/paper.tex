...
\textbf{Homomorphic Tallying}: Once voting ends, counting is specified as an inductive state machine \texttt{count}, parameterised 
...
The Helios verifier is implemented in OCaml and extracted from our 
Rocq specifications. It operates in two stages: (i) The verifier reads election data in JSON format from stdin, parsing ballots, 
tallier decryption factors, and proofs using a custom parser and lexer. The AST module translates JSON into OCaml data structures corresponding to Rocq ballot and tallier records. The core verification logic is defined in (src/Backend/HeliosTally.v) and instantiated with concrete parameters in (src/Examples/HeliosTallyIns.v). The verifier implements a state machine with inductive rules corresponding to election phases.

\textbf{Verification Rules}: The election verification process follows a very similar approach described in the 
previous section. We have an inductive datatype \texttt{count}, index by \texttt{state}, defined in (src/Backend/HelioTally.v). 
...
elections stored in (src/Examples/HeliosTallyIns.v):

\begin{itemize}
\item \textbf{Prime Fields}: 2048-bit prime $p$ and 256-bit prime $q$ (with $p = kq + 1$ for safe prime 
structure). We have formally verified that these prime numbers are indeed prime using Coqprime \cite{thery2007primality}.
