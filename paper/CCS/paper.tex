...
%% article.
\begin{abstract}
  $\Sigma$-protocols remain a popular choice in electronic voting to maintain privacy 
  and verifiability of elections and are still used in prominent electronic voting systems such as 
  Helios, Belenois, and SwissPost.
  However, existing implementations of $\Sigma$-protocols have two 
  ...
  and (ii) none of these implementations provide formal security guarantees. While Belenois partially mitigates
  the first issue using $Js\_of\_ocaml$ to extract JavaScript from OCaml code, the resulting system 
  lacks end-to-end formal verification of the protocols' security properties.
  
  In this paper, we present a certified formalization of a comprehensive library of $\Sigma$-protocols 
  in the Rocq theorem prover, including Schnorr, Chaum-Pedersen, Okamoto, And-$\Sigma$, Or-$\Sigma$, 
  Parallel-$\Sigma$, Eq-$\Sigma$, Neq-$\Sigma$ ($\neq$), Pedersen linear relation, 
  encryption proof protocols. For each protocol, we formally prove (special) soundness, completeness, and 
  (special honest-verifier) zero-knowledge properties. Our generic formalization over abstract vector spaces 
  enables code extraction and compilation to executable implementations in OCaml and WebAssembly,
  eliminating code duplication across platforms.
  We demonstrate the practical applicability of our framework by implementing verifiers 
  for privacy-preserving applications, including the IACR 2024 election tallysheet.
  Our certified cryptographic library provides a foundation for building formally verified 
...
the formal language of the Coq proof assistant.  We have formalised and analysed the interactive version of 
...
a cryptographic hash function, though some care is required \cite{10.1007/978-3-642-34961-4_38}.
...
the front-end (WebAssembly code) and back-end (Rust code) simultaneously. 
...
\subsection{Helios Verifier}
  We use our formalisation to develop an IACR election 
  verifier. Helios uses the Approval voting method in IACR 
  election. In the Approval voting method, each voter 
  either picks a candidate or don't. In case, it pick 
  the ballot become $(g^r, g^1 \cdot h ^ r)$ and 
  if not, then it becomes $(g^r, g^0 \cdot h ^ r)$. 
  In addition, the voter also provides a 
  zero-knowledge proof that their ballot 
  only contains either 0 or 1.  

  
  Each cast ballot in an IACR election 
  contains a list of ciphertext and 
  a corresponding zero-knowledge proof that they 
  are 
